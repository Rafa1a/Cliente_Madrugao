...
const ControlledTooltip_ultimos_pedidos: React.FC<any> = (props) => {
  ...
  useEffect(() => {
    const updatedTutorials = props.tutorials?.map((tutorial:any) => {
      if (tutorial.value === 'ultimos_pedidos') {
...
export default connect(null, mapDispatchProps)(ControlledTooltip_ultimos_pedidos);